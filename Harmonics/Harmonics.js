...
function draw() {
  ...
  //console.log(yy[0],yy[1]);
  circle(yy[0]*10+width/2,(height-graafikute_kõrgus)/2,20);
    push();
    fill(255);
    text("nihe: "+nihe+"m",width/2,30);
    pop();
  ...
  push();
  fill(255);
  text("x: "+round_2(yy[0]),30,40);
  text("v: "+round_2(yy[1]),30,70);
  text("v: "+round_2(yp[0]),30,100);
  text("a: "+round_2(yp[1]),30,130);
  pop();
  
  ekin=0.5*m*yy[1]*yy[1];
  epot=0.5*k_Hooke*yy[0]*yy[0];
  etot=ekin+epot;
  //console.log(yy[0],yy[1],yp[0],yp[1]);

  append(aja_massiiv,it);
  append(Epot_massiiv,epot);
  append(Ekin_massiiv,ekin);
  append(Etot_massiiv,etot); 
...
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if (abs((Etot_massiiv[k]/10)/plot_scale) <= graafikute_kõrgus/2 ) {
    ...
    stroke(255,125,0);
    strokeWeight(1);
    point((aja_massiiv[k]*amplifier_t)+plot_offset_X, -(Etot_massiiv[k]/10)/plot_scale+(height-plot_offset_Y));
    //console.log(min(Etot_massiiv));
    pop();
    if (k>=1) {
      stroke(255,125,0);
      strokeWeight(1);
      line((aja_massiiv[k-1]*amplifier_t)+plot_offset_X, -(Etot_massiiv[k-1]/10)/plot_scale+(height-plot_offset_Y) ,(aja_massiiv[k]*amplifier_t)+plot_offset_X , -(Etot_massiiv[k]/10)/plot_scale+(height-plot_offset_Y) );
  };
  };
};
pop();      
...
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if (abs((Ekin_massiiv[k]/10)/plot_scale) <= graafikute_kõrgus/2 ) {
    ...
    stroke(255,125,0);
    strokeWeight(1);
    point((aja_massiiv[k]*amplifier_t)+plot_offset_X, -(Ekin_massiiv[k]/10)/plot_scale+(height-plot_offset_Y));
    //console.log(min(Etot_massiiv));
    pop();
    if (k>=1) {
      stroke(255,125,0);
      strokeWeight(1);
      line((aja_massiiv[k-1]*amplifier_t)+plot_offset_X, -(Ekin_massiiv[k-1]/10)/plot_scale+(height-plot_offset_Y) ,(aja_massiiv[k]*amplifier_t)+plot_offset_X , -(Ekin_massiiv[k]/10)/plot_scale+(height-plot_offset_Y) );
  };
  };
};
pop();      
...
for (var k=0; k<=aja_massiiv.length; k=k+1) {
  if (abs((Epot_massiiv[k]/10)/plot_scale) <= graafikute_kõrgus/2 ) {
    ...
    stroke(255,125,0);
    strokeWeight(1);
    point((aja_massiiv[k]*amplifier_t)+plot_offset_X, -(Epot_massiiv[k]/10)/plot_scale+(height-plot_offset_Y));
    //console.log(min(Etot_massiiv));
    pop();
    if (k>=1) {
      stroke(255,125,0);
      strokeWeight(1);
      line((aja_massiiv[k-1]*amplifier_t)+plot_offset_X, -(Epot_massiiv[k-1]/10)/plot_scale+(height-plot_offset_Y) ,(aja_massiiv[k]*amplifier_t)+plot_offset_X , -(Ekin_massiiv[k]/10)/plot_scale+(height-plot_offset_Y) );
  };
  };
};
pop();      
